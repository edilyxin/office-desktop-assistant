...
from PyQt5.QtWidgets import (
    ...
    QMessageBox, QAction, QMenuBar, QStatusBar, QGroupBox, QCheckBox,
        ...
        try:
            if self.file_path:
                ...
                result = self.ocr_client.recognize(
                    self.file_path,
                    use_doc_orientation_classify=self.options.get('use_doc_orientation_classify', False),
                    use_doc_unwarping=self.options.get('use_doc_unwarping', False),
                    use_chart_recognition=self.options.get('use_chart_recognition', False),
...
class PaddleOCRVLAssistant(QMainWindow):
    ...
    def start_ocr_recognition(self, file_path=None, image_bytes=None):
        ...
        if self.current_image_bytes:
            ...
            logger.debug(f"图片显示完成，原始尺寸: {pixmap.size()}, 缩放后尺寸: {scaled_pixmap.size()}")
        ...
        # 创建并启动OCR线程
        self.ocr_thread = OCRThread(self.ocr_client, file_path, image_bytes, options)
    ...
    def process_images(self, ocr_result, markdown_text):
        ...
        for i, res in enumerate(ocr_result["layoutParsingResults"]):
            ...
            for img_path, img_url in res["markdown"]["images"].items():
                # 构建本地图片路径
                local_img_path = os.path.join(imgs_dir, os.path.basename(img_path))
                ...
                try:
                    ...
                    if img_response.status_code == 200:
                        ...
                        markdown_text = markdown_text.replace(img_path, local_img_path)
                except Exception as e:
...
'''
    ...
    def copy_to_clipboard(self):
        ...
        if self.current_ocr_result:
            markdown_text = self.markdown_manager.extract_markdown_text(self.current_ocr_result)
        ...
        if self.current_ocr_result:
            ...
            if output_dir:
                try:
                    ...
                    # 保存文件
                    saved_files = self.markdown_manager.save_markdown(self.current_ocr_result, output_dir)
                    logger.info(f"已保存 {len(saved_files)} 个文件到: {output_dir}")
                    self.status_bar.showMessage(f"已保存 {len(saved_files)} 个文件")
                    QMessageBox.information(self, "保存成功", f"已保存到: {output_dir}")
                except Exception as e:
    ...
    def upload_target_file(self):
        ...
        if file_path:
            # 更新显示
            self.target_label.setText(f"已选择目标文件: {os.path.basename(file_path)}")
    ...
    def process_file(self):
        ...
        try:
            ...
            logger.info(f"已打开模板文件: {self.template_path}")
            
            # 打开目标文件
            target_doc = Document(self.target_path)
            logger.info(f"已打开目标文件: {self.target_path}")
            
            # 1. 分析模板文档的样式信息
            logger.info("开始分析模板文档样式...")
            
            # 1.1 分析段落样式
            template_paragraph_styles = []
            for i, template_paragraph in enumerate(template_doc.paragraphs):
                style_name = template_paragraph.style.name
                logger.debug(f"模板段落 {i} 文本: {template_paragraph.text}")
                template_paragraph_styles.append(style_name)
                if style_name != 'Normal':
                    logger.debug(f"模板段落 {i} 样式: {style_name}")
            logger.info(f"模板文档共有 {len(template_doc.paragraphs)} 个段落")
            logger.info(f"模板段落样式统计: {dict((x, template_paragraph_styles.count(x)) for x in set(template_paragraph_styles))}")
            
            # 1.2 分析表格样式
            template_table_styles = []
            for i, template_table in enumerate(template_doc.tables):
                style_name = template_table.style.name
                template_table_styles.append(style_name)
                if style_name != 'Normal Table':
                    logger.debug(f"模板表格 {i} 样式: {style_name}")
            logger.info(f"模板文档共有 {len(template_doc.tables)} 个表格")
            logger.info(f"模板表格样式统计: {dict((x, template_table_styles.count(x)) for x in set(template_table_styles))}")
            
            # 2. 处理段落样式
            logger.info("开始处理段落样式...")
            modified_paragraphs = 0
            for i, target_paragraph in enumerate(target_doc.paragraphs):
                original_style = target_paragraph.style.name
                # 如果模板中有足够的段落，复制样式
                if i < len(template_doc.paragraphs):
                    template_paragraph = template_doc.paragraphs[i]
                    template_style = template_paragraph.style.name
                    # 复制段落样式名称
                    if template_style != 'Normal':
                        try:
                            target_paragraph.style = target_doc.styles[template_style]
                            if original_style != template_style:
                                modified_paragraphs += 1
                                logger.debug(f"修改段落 {i} 样式: {original_style} -> {template_style}")
                        except KeyError:
                            # 如果样式不存在，忽略
                            logger.warning(f"目标文档中不存在样式: {template_style}")
            logger.info(f"共处理 {len(target_doc.paragraphs)} 个段落，修改了 {modified_paragraphs} 个段落的样式")
            
            # 3. 处理表格样式
            logger.info("开始处理表格样式...")
            modified_tables = 0
            for i, target_table in enumerate(target_doc.tables):
                original_style = target_table.style.name
                # 如果模板中有足够的表格，复制样式
                if i < len(template_doc.tables):
                    template_table = template_doc.tables[i]
                    template_style = template_table.style.name
                    # 复制表格样式
                    if template_style != 'Normal Table':
                        try:
                            target_table.style = target_doc.styles[template_style]
                            if original_style != template_style:
                                modified_tables += 1
                                logger.debug(f"修改表格 {i} 样式: {original_style} -> {template_style}")
                        except KeyError:
                            # 如果样式不存在，忽略
                            logger.warning(f"目标文档中不存在样式: {template_style}")
            logger.info(f"共处理 {len(target_doc.tables)} 个表格，修改了 {modified_tables} 个表格的样式")
            ...
            # 保存文件
            target_doc.save(self.processed_path)
            logger.info(f"已保存处理后的文件: {self.processed_path}")
            
            self.style_result_label.setText(f"文件处理完成!\n结果文件: {processed_name}")
